...
战争后续顺序按用户最新决定为：敌方互攻与战争节奏先行，战况系统随后消费其实际结果。真名隐藏、代号遮蔽及识破流程从圣杯战争范围删除；不更改依赖提供的角色真名展示。互攻方案已冻结：真实地点绑定、场外回合、玩家介入、离图续战和从者独立作战；本批开始实现，会话状态由战争 GameComponent 持久化。

2026-09-07 用户明确修订：供魔设施、复杂工坊与 Caster 多工坊机制属于魔术模块，不在圣杯战争模块实现，也不作为本模块下一切片。圣杯战争继续负责现有简易战争 Site 的进攻、撤退、逃脱重建及当前契约归属协作；不删除已验收据点行为。魔术模块的设施、资源生产、防御经营与多工坊规则独立设计，实际接入时复用现有契约查询和撤退策略边界，不提前创建无消费者的接口或状态。
