...
灵体隐形 Hediff、能否工作、能否攻击等效果均由 `presenceState` 派生，不增加持久字段。
...
灵体状态切片已经实现：御主持有的状态命令、同图转换校验、复用 Core 隐形组件的表现 Hediff、灵体止血、根据权威状态定期校正派生效果、冻结 Pawn Job，以及攻击和能力入口门控。该切片没有增加持久状态。
