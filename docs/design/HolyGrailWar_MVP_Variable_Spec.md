...
13. 按切片 4 最终合同，御主死亡/失格解除有效契约并立即断供，从者保留库存，默认落单存续一天；能力优先通过原版 Stat 延长，跨图、场外、旅行和读档不重置。旧断供链不得提前清除或绕过该期限。实体化从者仍可承受环境伤害，免疫疾病、衰老和饥饿归零后果。
...
| 剩余令咒及资格 | 右手 `Hediff_CommandSpell` 的 severity；资格由有效印记派生 |
...
令咒使用 `Hediff_CommandSpell : Hediff_Implant`，只绑定可明确识别的右手。`CompMasterCommandSpells` 保留旧类型与调用入口，不再保存运行时划数。完整最终合同见 [切片 4](HolyGrailWar_Expansion_Plan.md)，以下字段切换为第一批范围，失契、手术、重签与胜负各自验收。

| 字段 | 类型 | 存档 | 默认值 | 说明 |
|---|---|---:|---:|---|
| `MW_CommandSpellMark.severity` | 原版 Hediff severity | 原版保存 | 无印记 | 唯一剩余划数，正式授予 3；有效范围 1 到 3，耗尽移除。 |
| `commandSpellCharges` | 旧 int | 迁移输入；失败时原值保留 | 旧默认 3 | 成功后不再写回。异常且无印记时保留原值供下次读档重试，不作为运行时划数或资格；无旧资格者不因默认值获得印记。 |
| `commandSpellHealthMigrated` | bool | 是 | 新 Pawn 为 true，旧档缺键 false | 防止移除、耗尽、补肢或再次读档重新迁移；不表达划数或资格。 |
| `MW_NoblePhantasmOvercharge` | 目标从者 Hediff | 原版保存 | 无 | 等待目标从者下一次成功释放 MoonWorld 宝具，无超时、不可叠加；不在御主组件保存 pending 布尔值。 |

令咒 Trait 仅留 Def 兼容并在迁移时移除，不授予资格。右手或连带右臂缺失会损毁印记，普通截肢/移植不产出令咒；补肢、清伤、治疗、复活不补发。专用摘取/植入另以原版物品保存转移途中的实际划数，成功转移才改变唯一持有者，不叠加、不复制。手术失败及已有印记接受者策略待手术批次集中确认。常规召唤仍需本届指定与未用资格，重签及移植不恢复资格；从者的 master 字段仍是唯一契约来源。
...
| 御主死亡/失格后的从者状态 | 解除有效契约，断供并进入默认一天落单存续；成功重签解除期限，失败不刷新。 |
...
1. 战争胜利或失败后，`warStartTick` 保留用于历史显示；战争 Quest 结束后停用本届战争事件，但不删除历史时间。
2. 从者在 caravan、运输舱、传送、地图撤离时的灵体状态如何表现？
3. 六维参数、额外具备神秘度的攻击来源及其他魔力来源如何扩展，而不改变当前 MVP 的存档模型？
4. 敌对御主死亡/失格后的从者按切片 4 进入落单存续，保留 Pawn、装备、伤势和魔力；从者撤退或 Site 被摧毁本身不等于资格丧失或从者消散。具体匹配与手术失败策略在对应批次集中确认。
...
当前运行时参战记录及结果继续由 Entry/GameComponent 保存，Quest 只展示并保留结束快照，Site 不保存契约、魔力或胜负副本。切片 4 明确四项独立事实：右手印记资格、有效契约/退场待重签、从者存续、当前职阶阵营。仍有印记的无从者御主不设自动失格期限；重签沿用原 Pawn、取消该次落单期限、改属新从者职阶阵营，历史身份保留。胜利按本届当前敌对从者存续判断，覆盖地图、旅行、场外和落单；御主死亡/失格及旧 enemy 淘汰标记不代替从者消散，无敌对从者时剩余退场御主不阻塞结束。玩家与敌方必须统一接入，匹配和玩家确认 UI 尚未冻结。
