...
4. 敌对御主被击杀后，其从者的具体退场状态和装备处理规则是什么；从者撤退或 Site 被摧毁本身不等于御主失去资格。
...
后续多阵营战争以 Quest 持有阵营记录为生命周期权威，每个阵营记录包含职阶席位、御主资格、有效从者契约集合和多个原版 Site 引用。Site 只表达据点位置与存在，不保存契约、魔力或胜负副本；Pawn/CompServantState 仍是角色和契约事实的权威。御主死亡、永久失去继续签约能力等条件才会令阵营失去资格；从者离图、战败灵体化或 Site 被摧毁后成功逃脱，只触发撤退/重建流程。战争胜利由 Quest 派生为所有敌方阵营均失去资格，不能用“当前地图没有敌方从者”替代。当前 MVP 不创建上述阵营、Site 集合或重新签约字段。
