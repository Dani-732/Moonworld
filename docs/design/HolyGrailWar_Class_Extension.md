...
切片 4 的 [最终规则](HolyGrailWar_Expansion_Plan.md) 替代旧资格简化：只有从者真正死亡/彻底消散才算对应阵营战败，御主死亡或失格只使其失契落单。仍有右手令咒的无从者御主保留资格、退场待重签；重签改属新从者职阶阵营，历史席位不被覆盖。战争胜利按本届当前敌对从者存续计算，不能永久按初始敌方列表或御主死亡结算。此为设计合同，逐批实施与验收另行记录。
