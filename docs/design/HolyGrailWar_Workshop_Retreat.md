...
2026-09-07 用户划分：供魔设施、复杂工坊及 Caster 多工坊机制归魔术模块。本文仅保留已实现的简易战争据点、撤退与重建协作，不把设施经营或多工坊开发列为圣杯战争后续切片；外部模块接管决策时仍遵守下述原 Pawn、真实逃脱及唯一状态边界。
...
7. 从下令撤退至新 Site 建成期间该方不能发动突袭；其余阵营按原规则活动。任务界面显示“工坊失守，等待休整并重建”。按 [切片 4 最终规则](HolyGrailWar_Expansion_Plan.md)，御主死亡或失格使从者失契落单，不等于阵营战败；只有从者真正死亡/消散才退场。旧主从重建仍要求有效契约及真实逃脱，不能借重建复活或补资源；重签后的工坊归属需随当前职阶阵营同步，保留历史。
